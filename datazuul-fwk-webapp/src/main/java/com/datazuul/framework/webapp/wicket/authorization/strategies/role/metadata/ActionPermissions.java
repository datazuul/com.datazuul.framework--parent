...
/**
 * For each Action, holds a set of roles that can perform that action. Roles can be granted access to a given action via
 * authorize(Action, String role) and denied access via unauthorize(Action, String role). All permissions can be removed
 * for a given action via authorizeAll(Action).
 *
...
final class ActionPermissions implements IClusterable {
    ...
    /**
     * Remove the given authorized role from an action. Note that this is only relevant if a role was previously
     * authorized for that action. If no roles where previously authorized the effect of the unauthorize call is that no
     * roles at all will be authorized for that action.
     *
     * @param action The action
     * @param rolesToRemove The comma separated list of roles to remove
     */
