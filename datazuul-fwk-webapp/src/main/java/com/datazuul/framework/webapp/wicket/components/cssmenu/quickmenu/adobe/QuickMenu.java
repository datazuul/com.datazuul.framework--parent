package com.datazuul.framework.webapp.wicket.components.cssmenu.quickmenu.adobe;
...
public class QuickMenu extends Panel {
    ...
    public QuickMenu(final String id, final List<MenuItem> topLevelEntries) {
        ...
        final WebMarkupContainer left = new WebMarkupContainer("left");
        final WebMarkupContainer right = new WebMarkupContainer("right");

        left.add(AttributeModifier.replace("style",
                "font-size:1px;width:6px;height:34px;background-image:url(resources/" + getClass().getName()
                + "/left_cap.gif)"));
        right.add(AttributeModifier.replace("style",
                "font-size:1px;width:6px;height:34px;background-image:url(resources/" + getClass().getName()
                + "/right_cap.gif)"));

        div.add(AttributeModifier.replace("style", "background-image:url(resources/" + getClass().getName()
                + "/center_tile.gif)"));
        div.add(left);
        div.add(right);

        div.add(new SubMenuListView("topmenuitems", new PropertyModel(this, "topMenuItems")));
        add(div);
    }
